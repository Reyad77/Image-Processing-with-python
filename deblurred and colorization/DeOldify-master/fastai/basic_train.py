...
class Learner():
    ...
    def load(self, file:PathLikeOrBinaryStream=None, device:torch.device=None, strict:bool=True,
        ...
        try:
            ...
            # On newer PyTorch versions torch.load may default to weights_only=True
            # causing an UnpicklingError for older checkpoints that contain objects.
            # Retry loading with weights_only=False for trusted local model files.
            try:
                state = torch.load(source, map_location=device, weights_only=False)
            except Exception:
                # If that still fails, re-raise the original exception to preserve traceback.
                raise
    ...
    def purge(self, clear_opt:bool=True):
        ...
        try:
            ...
            # Retry for older saved states that require full unpickling
